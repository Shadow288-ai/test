...
DROP TRIGGER IF EXISTS update_client_portfolios_updated_at ON public.client_portfolios;
...
DROP POLICY IF EXISTS "Users can view their own portfolios" ON public.client_portfolios;
CREATE POLICY "Users can view their own portfolios"
  ON public.client_portfolios FOR SELECT
  TO authenticated
  USING (auth.uid() = client_id);

DROP POLICY IF EXISTS "Users can create their own portfolios" ON public.client_portfolios;
CREATE POLICY "Users can create their own portfolios"
  ON public.client_portfolios FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = client_id);

DROP POLICY IF EXISTS "Users can update their own portfolios" ON public.client_portfolios;
CREATE POLICY "Users can update their own portfolios"
  ON public.client_portfolios FOR UPDATE
  TO authenticated
  USING (auth.uid() = client_id);

DROP POLICY IF EXISTS "Users can delete their own portfolios" ON public.client_portfolios;
CREATE POLICY "Users can delete their own portfolios"
  ON public.client_portfolios FOR DELETE
  TO authenticated
  USING (auth.uid() = client_id);

DROP POLICY IF EXISTS "Admins can view all portfolios" ON public.client_portfolios;
CREATE POLICY "Admins can view all portfolios"
  ON public.client_portfolios FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can manage all portfolios" ON public.client_portfolios;
...
DROP POLICY IF EXISTS "Admins can manage all holdings" ON public.portfolio_holdings;
...
CREATE INDEX IF NOT EXISTS idx_client_portfolios_client_id ON public.client_portfolios(client_id);
...
CREATE INDEX IF NOT EXISTS idx_portfolio_holdings_sector ON public.portfolio_holdings(sector);
CREATE INDEX IF NOT EXISTS idx_portfolio_holdings_region ON public.portfolio_holdings(region);
